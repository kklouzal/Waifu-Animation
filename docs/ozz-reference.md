...
- `AnimationSamplingContext` caches ordinary track intervals rather than Ozz's compressed GV4 iframe entries and SoA hot data. It is deterministic and coherent for forward playback, but it intentionally does not mirror Ozz allocation, archive, or packed key-controller memory layout.
