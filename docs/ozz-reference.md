...
- Root motion sampling follows the Ozz motion extraction/playback split: callers explicitly choose a carrier joint, sample its local transform with normal clip sampling, then consume interval deltas outside renderer bone mutation. `AnimationRuntime.update(deltaSeconds, { collectRootMotion: true })` exposes a blended override-layer motion delta without applying that motion to skeleton poses. Waifu does not currently perform Ozz's offline bake/removal pass.
