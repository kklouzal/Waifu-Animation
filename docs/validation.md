...
- root-motion authority/reconciliation for physics-driven, animation-driven, and hybrid modes; carrier none/layer/clip/bone/runtime-report selection priority and ties; local-vs-world conversion; collision partial consumption/residual reports; invalid/throwing adapter hardening; no-gain adapter clamping; ownership-token no-double-apply state including disabled token-history restore boundaries; stale carrier/source-basis layer replacement regression coverage; snapshot/replay equality; and deterministic coordinator root-motion report batching.
...
`AnimationRuntime.update(deltaSeconds, { collectRootMotion: true })` returns a finite explicit `rootMotionDelta` plus deterministic per-layer contribution metadata for active override layers. Additive layers are excluded from root-motion collection, masks scale carrier ownership, and invalid/non-finite timing resolves to identity motion. Replacing a runtime layer with a different clip id no longer inherits prior clip-specific motion-carrier or source-basis metadata unless the caller explicitly supplies it again; replacing the same clip id preserves the legacy compatibility behavior, and callers may force clearing with `clearMotionCarrier` / `clearSourceBasisQuaternion`.
...
Root-motion manifest metadata is intentionally split between policy and provenance. `source.rootMotion.policy` says how consumers should treat the runtime clip (`none`, `preserved`, or `stripped-to-in-place`). Optional `source.rootMotion.provenance` records how the root-carrier translation reached that state (`not-authored`, `preserved-in-clip`, `stripped-during-conversion`, or `requires-runtime-stripping`). Legacy manifests that only set `{ "policy": "stripped-to-in-place" }` remain readable and report `rootMotionProvenance: "unknown"`; asset reports also include root-carrier translation track counts so consumers can distinguish current binary state from missing provenance. Moving root/hips yaw is now validated alongside root-carrier translation: if a clip keeps carrier yaw while stripping translation, declare the mixed ownership explicitly with `source.rootMotion.yawPolicy: "preserved"` (or the clip metadata fallback `rootMotionYawPolicy`).

Three runtime clip setup applies the translation and yaw channel policies independently after UUID binding. `translationPolicy: "stripped-to-in-place"` removes root/hips/pelvis position tracks except for the explicitly declared residual pelvis carrier cases; `yawPolicy: "stripped-to-in-place"` removes root/hips/pelvis quaternion carrier tracks. Declare `yawPolicy: "preserved"` when the skeleton path intentionally keeps carrier yaw so controller/model-root application can make an explicit ownership decision instead of double-applying actor yaw.

Importer plans are non-mutating unless motion extraction is explicit. A missing `motion`/`rootMotion` section disables extraction; `motion: {}` or `motion: true` opts into the Ozz-style defaults. Per-channel `axes: "none"` disables that channel instead of creating empty translation tracks or falling back to yaw.

`.waifuanim.bin` payloads preserve JSON-compatible `AnimationClip.metadata`, including root-motion policy/provenance and carrier authority hints, so binary roundtrips do not lose fallback policy metadata. Manifest `source.rootMotion` remains authoritative when present, and `RootMotionReconciler` still requires explicit runtime carrier and ownership-token declarations before any world/model application.
