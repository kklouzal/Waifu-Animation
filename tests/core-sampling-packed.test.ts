...
export async function runCoreSamplingPackedTests(rawAnimationFixtures: {
  ...
  assert.equal(
    coherentSamplingContext.snapshot().lastMode,
    "reset",
    "first coherent sample should reset the context for the animation"
  );
  assert.equal(
    coherentSamplingContext.snapshot().maxTracks,
    coherentSamplingClip.tracks.length,
    "sampling context should resize to fit track count"
  );
  assert.ok(
    Math.abs(coherentFirstPose[2]!.translation[0] - 0.5) < 1e-6,
    "ratio sampling should convert clamped ratios to clip time"
  );
  ...
  assert.equal(
    coherentReuseSnapshot.lastMode,
    "coherent-forward",
    "increasing samples should use coherent forward mode"
  );
  assert.ok(
    coherentReuseSnapshot.reusedTrackCount >= 1,
    "coherent forward sampling should reuse cached intervals when the sample stays inside them"
  );
  assert.equal(
    coherentReuseSnapshot.searchedTrackCount,
    0,
    "coherent forward sampling should avoid binary seeking inside cached intervals"
  );
  ...
  assert.ok(
    coherentAdvanceSnapshot.advancedTrackCount >= 1,
    "coherent forward sampling should advance cached intervals when crossing keys"
  );
  ...
  assert.equal(
    coherentSeekSnapshot.lastMode,
    "seek",
    "backward sampling should seek instead of reusing stale forward intervals"
  );
  assert.ok(
    coherentSeekSnapshot.searchedTrackCount >= 1,
    "backward sampling should refresh intervals with a bounded search"
  );
  ...
  assert.ok(
    Math.abs(coherentLoopPose[2]!.translation[0] - 0.5) < 1e-6,
    "time sampling with loop should preserve existing wrapping semantics"
  );
  ...
  assert.deepEqual(
    ...
    [0, 0, 0],
    "non-finite ratios should clamp to the first sample"
  );
  ...
  const contextFunctionPose = sampleClipToPoseWithContext(skeleton, coherentSamplingClip, 1, coherentSamplingContext, {
    loop: false
  });
  assert.deepEqual(
    contextFunctionPose[2]!.translation,
    [2, 0, 0],
    "standalone context sampling should match ordinary time sampling"
  );
    ...
    tracks: [
      {
        ...
        values: toFloat32Array([Number.NaN, Infinity, 1, 2, 3, 4])
      },
  ...
  assert.deepEqual(
    repairedContextPose[2]!.translation,
    [0, 0, 1],
    "context sampling should use the same finite vector repair path as sampleTrack"
  );
  assert.deepEqual(
  ...
  assert.deepEqual(
    coherentSamplingStats.perTrack.map((track) => [
      track.track,
      track.normalizedProperty,
      track.keyCount,
      track.jointIndex
    ]),
    [
  ...
  assert.equal(
    Object.isFrozen(packedRuntimeAnimation.times),
    true,
    "packed runtime time buffers should be immutable arrays"
  );
  assert.equal(
    Object.isFrozen(packedRuntimeAnimation.keyControllers[0]),
    true,
    "packed key-controller metadata should be immutable"
  );
  ...
  assert.deepEqual(
    packedRuntimeAnimation.keyControllers.map(
      (controller) =>
        `${controller.joint ?? controller.humanBone}.${controller.normalizedProperty}:${controller.sourceTrack}`
    ),
  ...
  assert.deepEqual(
    packedRuntimeAnimation.iframeTable.times,
    [0, 0.5, 1, 1.5, 2],
    "packed iframe table should collect unique sorted animation key times"
  );
  assert.deepEqual(
  ...
  const singleKeyPacked = buildPackedRuntimeAnimation(
    {
      ...
      duration: 1,
      tracks: [
        { humanBone: "head", property: "translation", times: toFloat32Array([0.25]), values: toFloat32Array([1, 2, 3]) }
      ]
    },
    skeleton
  );
  assert.deepEqual(
    singleKeyPacked.keyControllers[0]!.seekTable,
    { iframeLowerKeys: [0], iframeUpperKeys: [0] },
    "single-key packed tracks should seek to their only key"
  );
  ...
  assert.deepEqual(
    packedRuntimeStats.perTrack.map((track) => [
      track.track,
      track.normalizedProperty,
      track.keyCount,
      track.jointIndex
  ...
  for (const time of [-0.25, 0, 0.25, 1, 1.75, 2.25]) {
    ...
    assert.ok(
      vectorNearlyEqual(packedPose[2]!.translation, ordinaryPose[2]!.translation, 1e-6),
      `packed head translation should match ordinary sampling at ${time}`
    );
    assert.ok(
      vectorNearlyEqual(packedPose[1]!.scale, ordinaryPose[1]!.scale, 1e-6),
      `packed spine scale should match ordinary sampling at ${time}`
    );
    assert.ok(
      quaternionNearlyEqual(packedPose[2]!.rotation, ordinaryPose[2]!.rotation, 1e-6),
      `packed head rotation should match ordinary sampling at ${time}`
    );
    assert.ok(
      quaternionNearlyEqual(packedPose[3]!.rotation, ordinaryPose[3]!.rotation, 1e-6),
      `packed arm rotation should match ordinary sampling at ${time}`
    );
  }
  ...
  assert.deepEqual(
    packedRatioPose[2]!.translation,
    ordinaryRatioPose[2]!.translation,
    "packed ratio sampling should clamp like ordinary ratio sampling"
  );
  ...
  assert.deepEqual(
    rawPackedPose[3]!.translation,
    rawBuiltPose[3]!.translation,
    "packed runtime animations should sample RawAnimation-built clips like ordinary clips"
  );
  assert.deepEqual(
    rawPackedPose[1]!.scale,
    rawBuiltPose[1]!.scale,
    "packed runtime animations should preserve raw-built scale tracks"
  );
  ...
  if (!emptyPackedBuild.ok)
    assert.ok(emptyPackedBuild.issues.some((issue) => issue.message === "clip has no transform tracks"));
  assert.throws(
  ...
  assert.equal(
    duplicatePackedBuild.ok,
    false,
    "packed runtime animation builder should reject duplicate resolved channels"
  );
  if (!duplicatePackedBuild.ok)
    assert.ok(duplicatePackedBuild.issues.some((issue) => issue.message.includes("duplicate target channel")));
  ...
  assert.ok(
    validatePackedRuntimeAnimation(invalidVersionPacked, skeleton).some(
      (issue) => issue.message === "packed runtime animation archive version is unsupported"
    ),
  ...
  const unsortedPacked = {
    ...
    keyControllers: [
      ...
      ...packedRuntimeAnimation.keyControllers.slice(2)
    ]
  } as unknown as typeof packedRuntimeAnimation;
  assert.ok(
    validatePackedRuntimeAnimation(unsortedPacked, skeleton).some(
      (issue) => issue.message === "packed key controllers must be sorted"
    ),
  ...
  assert.ok(
    validatePackedRuntimeAnimation(staleSeekTablePacked, skeleton).some(
      (issue) => issue.message === "packed seek table does not match key times"
    ),
  ...
  const stalePackedTargetKey = {
    ...packedRuntimeAnimation,
    keyControllers: packedRuntimeAnimation.keyControllers.map((controller, index) =>
      index === 1 ? { ...controller, targetKey: "stale-target" } : controller
    )
  } as unknown as typeof packedRuntimeAnimation;
  assert.ok(
    validatePackedRuntimeAnimation(stalePackedTargetKey, skeleton).some(
  ...
  assert.ok(
    overlappingPackedBufferIssues.some(
      (issue) => issue.message === "packed key controller time ranges must not overlap"
    ),
    "packed runtime validation should reject controllers that alias another controller's time buffer"
  );
  assert.ok(
    overlappingPackedBufferIssues.some(
      (issue) => issue.message === "packed key controller value ranges must not overlap"
    ),
  ...
  assert.ok(
    validatePackedRuntimeAnimation(packedRuntimeAnimation, wrongPackedSkeleton).some(
      (issue) => issue.message === "packed key controller does not map to skeleton"
    ),
  ...
  assert.deepEqual(
    packedRuntimeAnimation.times.slice(
      packedHeadTranslation.timeOffset,
      packedHeadTranslation.timeOffset + packedHeadTranslation.keyCount
    ),
    [0, 1, 2],
    "packed runtime animations should not alias source clip time arrays"
  );
  assert.equal(
    packedRuntimeAnimation.values[packedHeadTranslation.valueOffset + 4],
    2,
    "packed runtime animations should not alias source clip value arrays"
  );
  packedUnsortedClip.metadata!.source = "mutated";
  assert.equal(
    packedRuntimeAnimation.metadata?.source,
    "clip",
    "packed runtime metadata should be cloned from source clip metadata"
  );
  assert.throws(
    () => {
    ...
    TypeError,
    "packed runtime arrays should reject mutation"
  );
  assert.throws(
    () => {
    ...
    TypeError,
    "packed runtime metadata should reject mutation"
  );
}
