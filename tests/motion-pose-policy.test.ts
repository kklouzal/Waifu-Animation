...
export async function runMotionPosePolicyTests(): Promise<void> {
  ...
  assert.deepEqual(
    Array.from(
      createSubtreeJointMask(subtreeMaskSkeleton, ["missing", "leftArm"], {
  ...
  const armSubtreeBlend = blendPoses(
    skeleton,
    [{ pose: malformedMaskPose, weight: 1, mask: createSubtreeJointMask(skeleton, "leftUpperArm") }],
    { threshold: 0.01 }
  );
  assert.equal(armSubtreeBlend[1]!.translation[0], 0, "subtree masks should leave unselected parents on fallback pose");
  assert.equal(
    armSubtreeBlend[2]!.translation[0],
    0,
    "subtree masks should leave unselected siblings on fallback pose"
  );
  assert.equal(
    armSubtreeBlend[3]!.translation[0],
    8,
  ...
  assert.equal(
    malformedMaskedBlend[3]!.translation[0],
    0,
    "short partial masks should leave unspecified joints on fallback pose"
  );
  ...
  assert.equal(
    shortOverrideBlend[1]!.translation[0],
    7,
    "short override poses should still blend available child joints"
  );
  assert.deepEqual(
    shortOverrideBlend[2]!.translation,
    skeleton.restPose[2]!.translation,
    "short override poses should fall back missing joints to rest pose"
  );
  assert.deepEqual(
    shortOverrideBlend[3]!.translation,
    skeleton.restPose[3]!.translation,
    "short override poses should keep trailing missing joints on rest pose"
  );
  ...
  assert.equal(
    shortFallbackBlend[0]!.translation[0],
    9,
    "valid fallback joints should still be used when fallback pose is short"
  );
  assert.deepEqual(
    shortFallbackBlend[1]!.translation,
    skeleton.restPose[1]!.translation,
    "short fallback poses should use skeleton rest pose for missing joints"
  );
  assert.deepEqual(
    shortFallbackBlend[2]!.translation,
    skeleton.restPose[2]!.translation,
    "short fallback poses should use skeleton rest pose for missing threshold fallback"
  );
  ...
  assert.equal(
    invalidFallbackBlend[1]!.translation[0],
    8,
    "valid fallback joints should still be used when neighboring fallback joints are invalid"
  );
  ...
  assert.ok(
    Math.abs(overweightMaskedBlend[2]!.translation[0] - 40 / 3) < 1e-6,
  ...
  assert.ok(
    validatePose(skeleton, invalidLayerPose).some(
  ...
  const invalidBlend = blendPoses(skeleton, [{ pose: invalidLayerPose, weight: 1 }], {
    threshold: 0.01,
    fallbackPose: nonZeroFallbackPose
  });
  assert.ok(
    invalidBlend[1]!.translation.every(
      (value, index) => Math.abs(value - nonZeroFallbackPose[1]!.translation[index]!) < 1e-6
    ),
  ...
  assert.ok(
    Math.abs(invalidBlend[1]!.rotation[1] - nonZeroFallbackPose[1]!.rotation[1]) < 1e-12,
    "invalid layer transforms should keep fallback rotations"
  );
  assert.ok(
  ...
  assert.ok(
    Math.abs(Math.hypot(...invalidBlend[1]!.rotation) - 1) < 1e-12,
    "invalid layer output rotations should stay normalized"
  );
  ...
  assert.deepEqual(
    mixedValidityBlend[1]!.translation,
    validSameJointPose[1]!.translation,
    "valid same-joint layers should still own joints ignored from malformed layers"
  );
  assert.deepEqual(
    mixedValidityBlend[1]!.scale,
    validSameJointPose[1]!.scale,
    "valid same-joint layers should still blend scale normally"
  );
  assert.ok(
    Math.abs(mixedValidityBlend[1]!.rotation[0] - validSameJointPose[1]!.rotation[0]) < 1e-12,
    "valid same-joint layers should still blend rotations normally"
  );
  for (const transform of mixedValidityBlend) {
    ...
    assert.ok(
      Math.abs(Math.hypot(...transform.rotation) - 1) < 1e-12,
      "malformed override layers should not produce denormalized rotations"
    );
  }
  ...
  assert.equal(
    malformedAdditivePose[1]!.translation[0],
    0,
    "NaN mask entries should not affect additive pose application"
  );
  assert.equal(
    malformedAdditivePose[2]!.translation[0],
    0,
    "missing mask entries should not affect additive pose application"
  );
  assert.equal(
    malformedAdditivePose[3]!.translation[0],
    0,
    "short additive masks should leave unspecified joints unchanged"
  );
  ...
  assert.ok(
    invalidUnmaskedAdditivePose.every(
      (transform) =>
        transform.translation.every(Number.isFinite) &&
        transform.rotation.every(Number.isFinite) &&
        transform.scale.every(Number.isFinite)
  ...
  assert.deepEqual(
    shortAdditivePose[1]!.translation,
    skeleton.restPose[1]!.translation,
    "short additive samples should use rest deltas for missing joints"
  );
  assert.deepEqual(
    shortAdditivePose[2]!.translation,
    skeleton.restPose[2]!.translation,
    "short additive samples should leave trailing missing joints unchanged"
  );
  assert.throws(
  ...
  assert.deepEqual(
    signedScaleDeltaPose[0]!.scale,
    [2, 3, -0.5],
    "additive scale deltas should preserve finite negative rest-scale ratios"
  );
  const signedScaleAppliedPose = applyAdditivePose(signedScaleRestPose, signedScaleDeltaPose, 1);
  assert.deepEqual(
    signedScaleAppliedPose[0]!.scale,
    signedScaleSamplePose[0]!.scale,
    "positive additive scale weights should apply signed scale ratios"
  );
  const signedScaleSubtractedPose = applyAdditivePose(signedScaleSamplePose, signedScaleDeltaPose, -1);
  assert.deepEqual(
    signedScaleSubtractedPose[0]!.scale,
    signedScaleRestPose[0]!.scale,
    "negative additive scale weights should invert signed scale ratios"
  );
  ...
  const nanThresholdBlend = blendPoses(skeleton, [{ pose: sampled, weight: DEFAULT_BLEND_THRESHOLD * 0.5 }], {
    threshold: Number.NaN
  });
  assert.ok(nanThresholdBlend[2]!.rotation[0] > 0, "NaN thresholds should not discard finite weak layer influence");
  assert.ok(
    nanThresholdBlend[2]!.rotation[0] < sampled[2]!.rotation[0],
    "NaN thresholds should preserve rest-pose fallback for tiny weights"
  );
  ...
  const weakInvalidFallbackBlend = blendPoses(
    skeleton,
    [{ pose: weakLayerPose, weight: DEFAULT_BLEND_THRESHOLD * 0.5 }],
    {
  ...
  const infiniteThresholdBlend = blendPoses(skeleton, [{ pose: sampled, weight: DEFAULT_BLEND_THRESHOLD * 0.5 }], {
    threshold: Number.POSITIVE_INFINITY
  });
  assert.ok(
    infiniteThresholdBlend[2]!.rotation[0] > 0,
    "infinite thresholds should not discard finite weak layer influence"
  );
  assert.ok(
    infiniteThresholdBlend[2]!.rotation[0] < sampled[2]!.rotation[0],
    "infinite thresholds should preserve rest-pose fallback for tiny weights"
  );
      ...
      tracks: [
        {
          ...
          values: sanitizeQuaternionTrackValues([0.2, 0, 0, 0.98])
        }
      ]
    },
    0
      ...
      tracks: [
        {
          ...
          property: "quaternion",
          times: toFloat32Array([0]),
          values: sanitizeQuaternionTrackValues([0, 0.2, 0, 0.98])
        }
      ]
    },
    0
  );
  const weightedBlend = blendPoses(
    skeleton,
    [
      { pose: weightedPoseA, weight: 2 },
      { pose: weightedPoseB, weight: 1 }
    ],
    { threshold: 0.01 }
  );
  assert.ok(
    weightedBlend[2]!.rotation[0] > weightedBlend[2]!.rotation[1],
  ...
  const highPriorityHeadClip: AnimationClip = {
    ...
    duration: 1,
    tracks: [
      { humanBone: "head", property: "translation", times: toFloat32Array([0]), values: toFloat32Array([20, 0, 0]) }
    ]
  };
  ...
  assert.equal(
    samePriorityRuntimePose[2]!.translation[0],
    samePriorityExpectedPose[2]!.translation[0],
  ...
  assert.ok(
    Math.abs(maskedPriorityPose[1]!.translation[0] - 4) < 1e-6,
    "higher-priority masked layers should leave unowned joints on the lower-priority pose"
  );
  assert.ok(
    Math.abs(maskedPriorityPose[2]!.translation[0] - 20) < 1e-6,
  ...
  assert.ok(
    Math.abs(weakPriorityPose[2]!.translation[0] - 15) < 1e-6,
  ...
  assert.ok(
    Math.abs(nanThresholdRuntimePose[2]!.translation[0] - 15) < 1e-6,
  ...
  runtimeInfiniteThreshold.setLayer("weak-head", highPriorityHeadClip, {
    weight: 0.05,
    targetWeight: 0.05,
    priority: 10,
    mask
  ...
  assert.ok(
    Math.abs(infiniteThresholdRuntimePose[2]!.translation[0] - 15) < 1e-6,
    "runtime infinite blend thresholds should preserve weak-layer fallback behavior"
  );
  ...
  assert.ok(
    Math.abs(midCrossfade.localPose[2]!.translation[0] - 6) < 1e-6,
    "in-progress crossfade should normalize same-priority override weights"
  );
  runtimeCrossfade.update(20);
  const finishedCrossfade = runtimeCrossfade.evaluate();
  assert.equal(
    finishedCrossfade.activeLayers.some((layer) => layer.id === "old"),
    false,
    "crossfade should remove fully faded source layers"
  );
  assert.ok(
    Math.abs(finishedCrossfade.localPose[2]!.translation[0] - 10) < 1e-4,
  ...
  assert.equal(
    baseDuringAdditiveTarget.targetWeight,
    1,
    "additive crossfade should not retarget same-priority override base layers"
  );
  assert.equal(
    baseDuringAdditiveTarget.weight,
    1,
    "additive crossfade should not fade same-priority override base influence"
  );
  ...
  assert.ok(
    Math.abs(additiveTargetDuringFade.weight - 0.5) < 1e-6,
    "additive crossfade target should fade in independently"
  );
  assert.ok(
    Math.abs(midAdditiveTargetCrossfade.localPose[2]!.translation[0] - 2.5) < 1e-6,
    "additive crossfade target should compose on top of the base pose while fading"
  );
  ...
  assert.equal(
    baseAfterAdditiveTarget?.targetWeight,
    1,
    "additive crossfade should keep base target weight after completion"
  );
  assert.ok(Math.abs(additiveTargetAfterFade!.weight - 1) < 1e-4, "additive crossfade target should finish fading in");
  assert.ok(
    Math.abs(finishedAdditiveTargetCrossfade.localPose[2]!.translation[0] - 3) < 1e-4,
  ...
  assert.equal(
    replacedExistingAdditiveLayer?.blendMode,
    "additive",
    "crossfading an existing additive layer should preserve its blend mode by default"
  );
  assert.equal(
    baseDuringExistingAdditiveReplace?.targetWeight,
    1,
    "replacing an additive layer should not implicitly fade same-priority override layers"
  );
  assert.ok(
    Math.abs(replacedExistingAdditiveLayer.weight - 0.5) < 1e-6,
    "replaced additive layers should still honor resetTime fade-in"
  );
  assert.ok(
    Math.abs(replacedExistingAdditive.localPose[2]!.translation[0] - 4) < 1e-6,
  ...
  assert.ok(
    Math.abs(subtractiveAdditiveEvaluation.localPose[2]!.translation[0] - 8) < 1e-6,
    "negative additive runtime weights should subtract from the base pose"
  );
  ...
  const additiveNegativeFadeLayer = additiveNegativeFadeEvaluation.activeLayers.find(
    (layer) => layer.id === "fade-subtract"
  );
  assert.ok(additiveNegativeFadeLayer, "additive layers should remain active while fading toward a negative target");
  assert.ok(
    Math.abs(additiveNegativeFadeLayer.weight + 0.5) < 1e-6,
    "additive targetWeight should fade toward negative values"
  );
  ...
  assert.equal(
    negativeOverrideEvaluation.activeLayers.length,
    0,
    "negative override weights should sanitize to no influence"
  );
  ...
  assert.ok(
    Math.abs(maskedCrossfadePose[1]!.translation[0] - 4) < 1e-6,
    "masked crossfade should keep unowned joints on lower-priority fallback pose"
  );
  assert.ok(
    Math.abs(maskedCrossfadePose[2]!.translation[0] - 20) < 1e-4,
    "masked crossfade target should own masked joints after fade completion"
  );
  ...
  assert.equal(
    globalThumbMaskRule.lastIndex,
    0,
    "global regex track masks should leave caller regex state deterministic"
  );
  ...
  const baseAuthoredClip = makeAuthoredLoopClip("base-authored-upper", [
    "hips.position",
    "spine",
    "leftShoulder",
    "rightUpperArm",
  ...
  assert.deepEqual(
    applySourceTrackPolicy(sourcePropertyPolicyClip, { include: [/^head\.quaternion$/] }).tracks.map(
  ...
  assert.deepEqual(
    applySourceTrackPolicy(sourcePropertyPolicyClip, { exclude: [/^head\.position$/] }).tracks.map(
      (track) => `${track.humanBone ?? track.joint}.${track.property}`
    ),
  ...
  assert.deepEqual(
    applySourceTrackPolicy(baseAuthoredClip, AUTHORED_BASE_SOURCE_TRACK_POLICY).tracks.map(
      (track) => track.humanBone ?? track.joint
    ),
  ...
  assert.deepEqual(
    applySourceTrackPolicy(baseAuthoredClip, BASE_PROCEDURAL_SOURCE_TRACK_POLICY).tracks.map(
      (track) => track.humanBone ?? track.joint
    ),
    ["hips", "spine", "leftShoulder", "rightUpperArm", "leftHand", "leftUpperLeg"],
